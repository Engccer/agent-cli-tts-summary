...
# 공백뿐인 요약 파일은 "이 턴은 재생 없음"이다: 보관 없이 조용히 지우고 통과한다(macOS 판과 같은 계약).
# /tts-replay(tts-replay.ps1)가 직전 음성을 다시 튼 턴에 새 요약이 겹쳐 재생되지 않도록 이 파일을 미리 써 둔다.
#
...
if (Test-TtsSessionMuted) { exit 0 }

#!/usr/bin/env python3
"""Windows TTS 설정기(tts-config-set.ps1)가 macOS 판과 같은 계약을 지키는지 검증한다.
PowerShell이 있는 환경(Windows)에서만 돌고, 없으면 건너뛴다.
실행: python scripts/test_tts_config_set_windows.py
"""
...
class WindowsTtsConfigSetTests(unittest.TestCase):
    def setUp(self) -> None:
        self.home = Path(tempfile.mkdtemp(prefix="tts-config-set-win-"))
        self.addCleanup(shutil.rmtree, self.home, ignore_errors=True)
        self.hooks = self.home / "hooks"
        self.hooks.mkdir()
        for name in ("tts-config.ps1", "tts-config-set.ps1"):
            shutil.copy(WINDOWS / name, self.hooks / name)
        self.config = self.home / ".claude" / "TTS-Summary" / "tts-config.txt"
        self.config.parent.mkdir(parents=True)
        shutil.copy(WINDOWS / "tts-config.txt", self.config)

    def run_setter(self, *args: str) -> subprocess.CompletedProcess:
        env = dict(os.environ, USERPROFILE=str(self.home))
        return subprocess.run(
            [POWERSHELL, "-NoProfile", "-ExecutionPolicy", "Bypass",
             "-File", str(self.hooks / "tts-config-set.ps1"), *args],
            env=env, capture_output=True, text=True, encoding="utf-8", errors="replace")

    def value(self, key: str) -> str:
        for line in self.config.read_text(encoding="utf-8-sig").splitlines():
    ...
    def test_show_without_args(self) -> None:
        ...
        self.assertIn("속도 5(SAPI Rate 0)", completed.stdout)
    ...
    def test_toggle_and_set_values(self) -> None:
        ...
        self.assertIn("속도 7.5(SAPI Rate 5), 상세 3단계", completed.stdout)

    def test_preserves_comments_and_other_keys(self) -> None:
        before = self.config.read_text(encoding="utf-8-sig").splitlines()
        self.run_setter("speed", "8")
        after = self.config.read_text(encoding="utf-8-sig").splitlines()
    ...
    def test_bom_and_line_endings_are_preserved(self) -> None:
        self.config.write_bytes(b"\xef\xbb\xbf# \xec\xa3\xbc\xec\x84\x9d\r\nenabled=on\r\nspeed=5\r\n")
        for _ in range(3):
            self.assertEqual(self.run_setter("speed", "8").returncode, 0)
        self.assertEqual(self.config.read_bytes(),
                         b"\xef\xbb\xbf# \xec\xa3\xbc\xec\x84\x9d\r\nenabled=on\r\nspeed=8\r\n")
        self.config.write_bytes(b"enabled=on\nspeed=5\n")
        self.assertEqual(self.run_setter("off").returncode, 0)
        self.assertEqual(self.config.read_bytes(), b"enabled=off\nspeed=5\n")

    def test_single_argument_string_is_split(self) -> None:
        # 슬래시 명령은 "$ARGUMENTS" 한 덩어리로 넘긴다.
        self.assertEqual(self.run_setter("speed 8").returncode, 0)
        self.assertEqual(self.value("speed"), "8")
        completed = self.run_setter("")
        self.assertEqual(completed.returncode, 0)
        self.assertIn("속도 8(SAPI Rate 6)", completed.stdout)

    def test_interim_toggle(self) -> None:
        completed = self.run_setter("interim", "on")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(self.value("interim"), "on")
        self.assertIn("선택지·중간 보고 켬", completed.stdout)
        self.assertEqual(self.run_setter("interim", "off").returncode, 0)
        self.assertIn("선택지·중간 보고 끔", self.run_setter().stdout)

...
2. 설정 파일을 설치하고 요약 언어와 재생 provider를 사용자와 확인한다.
   ...
   - **상세 정도(verbosity)**: 요약 분량을 1~3단계로 정한다(1은 1~2문장, 2는 3~6문장, 3은 7문장 이상). 이 값이 실제로 반영되려면 아래 "선택 훅"의 설정 통지 훅(UserPromptSubmit)이 필요하다. Antigravity에는 이 이벤트가 없어 지침 문구로 분량을 고정한다(`agy` 1.1.25 실측: `SessionStart`·`Stop`만 발동).
...
4. 스크립트를 설치한다.
   - macOS Codex에는 `python3 scripts/install_codex_commands.py`로 설정·재생 명령도 설치한다. 기본 TTS 루프가 먼저 있어야 한다. 기존 설정과 훅 등록은 보존하고, 바뀌는 명령 파일은 백업한다. `/skills`에서 `codex-tts`·`codex-tts-replay`를 선택하거나 `$codex-tts off`·`$codex-tts-replay`로 호출한다. `/tts`라는 직접 슬래시 별칭과 Claude의 `!` 셸 전처리는 제공하지 않는다. 상세: `references/codex-commands.md`.
...
7. 끝까지 검증한다.
   ...
   - `interim=on`이면 `ASK_TTS_DRYRUN=1`로 질문 선택지 안내가 문장을 조립하는지, `BRIEFING_TTS_DRYRUN=1`로 중간 보고가 설정을 읽는지 확인한다. `TTS_REPLAY_DRYRUN=1`이면 `/tts-replay` 재생기가 소리 없이 고른 파일 경로만 출력하므로 최신 파일 선택을 무음으로 검증할 수 있다.
...
- **설정 통지 (UserPromptSubmit hook)**: 매 턴 설정 파일을 읽어 사용 여부와 상세 정도를 `[tts-config]`로 시작하는 한 줄로 에이전트에 알린다. Stop hook 시점에는 요약이 이미 쓰인 뒤라 `verbosity`를 반영할 수 없으므로 이 훅이 담당한다. `assets/windows/tts-config-context.ps1`, `assets/macos/tts-config-context.sh`. 통지 한 줄이 상세 정도별 문장 수(1~2 / 3~6 / 7 이상)까지 담으므로 지침 블록에는 분량 표가 없다. Antigravity에는 이 이벤트가 없다. 공식 문서가 대응 자리로 읽히는 `BeforeAgent`를 적지만 `agy` 1.1.25는 그것도 발동시키지 않는다(실측). 그래서 Antigravity의 분량은 지침 문구로 고정한다. Codex는 Windows·macOS 모두 `hookSpecificOutput.additionalContext` JSON으로 전달하며, Claude는 일반 텍스트를 유지한다. Windows는 `$AgentDirName = ".codex"`로 치환하고 `assets/hooks/codex.windows.hooks.json`의 UserPromptSubmit 등록을 병합한다. 현재 공식 계약은 일반 텍스트도 허용하지만, 기존 버전과의 호환을 위해 JSON을 사용한다. 설치 후 훅 신뢰 상태와 실제 추가 컨텍스트 전달을 확인한다(`references/windows.md`).
...
- `scripts/test_tts_config_context_windows.py`: Windows Codex JSON·Claude 평문, 상세 정도 변경·끔·세션 음소거·설정 누락과 요약 파일 불간섭을 검증한다.

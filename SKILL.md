...
4. 스크립트를 설치한다.
   ...
   - `$ConverterScript`/`CONVERTER_SCRIPT`에는 speech-toolkit TTS 스크립트의 절대 경로를 넣는다. speech-toolkit이 스킬로 설치돼 있으면(예: `~/.claude/skills/speech-toolkit`) 그 안의 `TTS/gemini_tts.py`·`TTS/elevenlabs_tts.py` 경로를 찾아 자동으로 치환하고, 없으면 사용자에게 알린 뒤 저장소( https://github.com/Engccer/speech-toolkit )를 클론해 그 경로를 쓴다.

...
- **설정 통지 (UserPromptSubmit hook)**: 매 턴 설정 파일을 읽어 사용 여부와 상세 정도를 `[tts-config]`로 시작하는 한 줄로 에이전트에 알린다. Stop hook 시점에는 요약이 이미 쓰인 뒤라 `verbosity`를 반영할 수 없으므로 이 훅이 담당한다. `assets/windows/tts-config-context.ps1`, `assets/macos/tts-config-context.sh`. 통지 한 줄이 상세 정도별 문장 수(1~2 / 3~6 / 7 이상)까지 담으므로 지침 블록에는 분량 표가 없다. Antigravity에는 이 이벤트가 없다. 공식 문서가 대응 자리로 읽히는 `BeforeAgent`를 적지만 `agy` 1.1.25는 그것도 발동시키지 않는다(실측). 그래서 Antigravity의 분량은 지침 문구로 고정한다. Codex는 일반 텍스트 stdout을 실패로 처리하므로 `hookSpecificOutput.additionalContext` JSON을 출력해야 하며, 이 분기는 macOS 판에만 있다. Windows Codex는 그 이벤트 지원 여부를 이 스킬이 검증하지 않아 훅 샘플에도 등록이 없다: 쓰려면 CLI의 훅 계약을 먼저 확인하고 `tts-config-context.ps1`에 JSON 분기를 더한다.

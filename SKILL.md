...
2. 설정 파일을 설치하고 요약 언어와 재생 provider를 사용자와 확인한다.
   ...
   - **상세 정도(verbosity)**: 요약 분량을 1~3단계로 정한다(1은 1~2문장, 2는 3~6문장, 3은 7문장 이상). 이 값이 실제로 반영되려면 아래 "선택 훅"의 설정 통지 훅(UserPromptSubmit)이 필요하다. Antigravity에는 그 이름의 이벤트가 없어(대응 자리는 `BeforeAgent`이며 아직 구현하지 않았다) 지침 문구로 분량을 고정한다.

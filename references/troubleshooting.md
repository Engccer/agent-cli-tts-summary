...
에이전트가 `tts-summary.txt`를 **교체**하려 해서 생긴다. Stop hook은 요약을 읽은 직후 임시 파일을 지우므로 다음 턴 시작 시점에는 파일이 없는데, 에이전트가 파일이 남아 있다고 가정하고 `Delete File` + `Add File`(또는 기존 파일 수정) 패턴으로 패치하면 첫 단계가 실패한다. 이어서 `Add File`로 다시 쓰면 요약 자체는 정상 생성되지만, "에러 발생 시 반드시 포함" 규칙 때문에 같은 실패 설명이 매 턴 TTS로 낭독되는 부수 효과가 생긴다(2026-08-12 Windows Codex `apply_patch` 실측).
...
실측 사례(2026-07-17): provider가 가리키던 speech-toolkit 사본이 다른 폴더로 이사한 뒤 경로를 갱신하지 않자, provider가 매턴 조용히 실패하며 며칠간 OS 내장 폴백으로만 재생되고 있었다. 폴백 덕에 요약은 계속 들려서 알아채기 어렵다. 고품질 음색이 갑자기 OS 내장 음색으로 바뀌었다면 가장 먼저 provider 로그와 `$ConverterScript` 경로 유효성을 확인한다.

...
- `UserPromptSubmit`은 macOS Codex에서 발동과 추가 컨텍스트 주입이 동작한다. `tts-config-context.sh`는 기존 버전과 호환되는 `hookSpecificOutput.additionalContext` JSON을 출력한다. 현재 공식 계약은 일반 텍스트 stdout도 허용한다.

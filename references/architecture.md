...
이 루프는 외부 TTS CLI나 앱에 런타임 의존하지 않는다. 재생은 OS 내장 기능(Windows SAPI/`System.Speech`, macOS `say`)이나 명시적으로 호출하는 Gemini API provider(speech-toolkit( https://github.com/Engccer/speech-toolkit )의 `TTS/gemini_tts.py`)만 사용한다. `assets/`의 템플릿은 모두 중립 이름(`stop-tts`, `TTS_NO_PLAY`)을 쓴다.
